      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 160, gap: 16 }}>
        <Card>
          <View className="flex-row items-center">
            ...
            <View className="ml-4 flex-1">
              <Text className="text-lg font-semibold text-[#101828]">{staff.name}</Text>
              <Text className="font-sans mt-0.5 text-sm text-gray-500">{staff.role ?? t('detail.defaultRole')}</Text>
              {staff.phone ? (
                <View className="mt-1 flex-row items-center">
                  <FontAwesome5 name="phone-alt" size={11} color="#9CA3AF" />
                  <Text className="font-sans ml-1.5 text-sm text-gray-500">{staff.phone}</Text>
                </View>
              ) : (
                <Text className="font-sans mt-1 text-sm text-gray-400">{t('detail.noPhone')}</Text>
              )}
            </View>
          </View>
            ...
            {staff.wage_type === 'per_piece' ? (
              <>
                <View className="flex-row items-center justify-between rounded-md bg-gray-50 p-3">
                  <Text className="font-sans text-sm text-gray-600">{t('detail.amountPerPant')}</Text>
                  <Text className="text-base font-bold text-[#101828]">
                    {formatCurrency(staff.wage_amount_pant ?? 0)}
                  </Text>
                </View>
                <View className="flex-row items-center justify-between rounded-md bg-gray-50 p-3">
                  <Text className="font-sans text-sm text-gray-600">{t('detail.amountPerShirt')}</Text>
                  <Text className="text-base font-bold text-[#101828]">
                    {formatCurrency(staff.wage_amount_shirt ?? 0)}
                  </Text>
                </View>
                <View className="flex-row items-center justify-between rounded-md bg-gray-50 p-3">
                  <Text className="font-sans text-sm text-gray-600">{t('detail.amountPerPair')}</Text>
                  <Text className="text-base font-bold text-[#101828]">
              ...
              <View className="flex-row items-center justify-between rounded-md bg-gray-50 p-3">
                <Text className="font-sans text-sm text-gray-600">{t('detail.wageAmount')}</Text>
                <Text className="text-base font-bold text-[#101828]">
                  {formatCurrency(staff.wage_amount)}
                  <Text className="font-sans text-xs text-gray-500">{WAGE_LABELS[staff.wage_type]}</Text>
                </Text>
              </View>
            )}
          </View>
        ...
        <View>
          <View className="mb-2 flex-row items-center">
            ...
            <Text className="ml-2 text-base font-semibold text-[#101828]">{t('detail.workEntries')}</Text>
          </View>
          ...
          {workEntries.length === 0 ? (
            ...
            <View className="gap-2">
              ...
              <View className="flex-row items-center justify-between rounded-md bg-emerald-50 p-3">
                <Text className="font-sans text-sm font-medium text-emerald-800">
                  {t('detail.workTotal')}
                </Text>
        ...
        <View>
          <View className="mb-2 flex-row items-center">
            ...
            <Text className="ml-2 text-base font-semibold text-[#101828]">{t('detail.assignedOrders')}</Text>
          </View>
          {orders.length === 0 ? (
            <EmptyState
              variant="compact"
            ...
            <View className="gap-2">
              {orders.map((o) => (
                <Card key={o.id}>
                  <View className="flex-row items-center justify-between">
                    <Text className="text-sm font-semibold text-[#101828]">#{o.order_number}</Text>
